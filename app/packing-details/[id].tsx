...
export default function PackingDetailsScreen() {
  const { id } = useLocalSearchParams(); // Get Order ID from URL
  const router = useRouter(); // Navigation

  const handlePassToDelivery = () => {
    Alert.alert("Alert", "Order moved to Delivery Stage", [
      {
        text: "OK",
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Button to move to Delivery */}
      <TouchableOpacity style={styles.button} onPress={handlePassToDelivery}>
        <Text style={styles.buttonText}>Pass to Delivery</Text>
      </TouchableOpacity>
    </View>
  );
}
