...
export default function DryerDetailsScreen() {
  const { id } = useLocalSearchParams(); // Get Order ID from URL
  const router = useRouter(); // Navigation

  const handlePassToIron = () => {
    Alert.alert("Alert", "Order moved to Ironing Stage", [
      {
        text: "OK",
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Button to move to Ironing */}
      <TouchableOpacity style={styles.button} onPress={handlePassToIron}>
        <Text style={styles.buttonText}>Pass to Iron</Text>
      </TouchableOpacity>
    </View>
  );
}
